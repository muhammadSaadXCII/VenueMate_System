...
class _HallRegistrationScreenState extends State<HallRegistrationScreen> {
  ...
  void _nextStep() { if (_currentStep < 4) setState(() => _currentStep++); }
    ...
    switch (_currentStep) {
      case 0:
        ...
        if (_data.phoneController.text.trim().length != 11) {
          _snack('Phone number must be exactly 11 digits.'); return false;
        }
        if (_data.emailController.text.trim().isEmpty) {
          _snack('Please enter your email.'); return false;
        }
        if (_data.cnicController.text.trim().isEmpty) {
          _snack('Please enter your CNIC.'); return false;
        }
        if (_data.cnicFront == null) {
          _snack('Please upload CNIC front side.'); return false;
        }
        if (_data.cnicBack == null) {
          _snack('Please upload CNIC back side.'); return false;
        }
        return true;
      case 1:
        if (_data.hallNameController.text.trim().isEmpty) {
          _snack('Please enter hall name.'); return false;
        }
        if (_data.rentController.text.trim().isEmpty ||
            double.tryParse(_data.rentController.text.trim()) == null) {
          _snack('Please enter a valid hall rent.'); return false;
        }
        if (_data.locationController.text.trim().isEmpty) {
          _snack('Please select a location.'); return false;
        }
        if (_data.minController.text.trim().isEmpty ||
            _data.maxController.text.trim().isEmpty) {
          _snack('Please enter guest capacity (min and max).'); return false;
        }
        return true;
      case 2:
        if (_data.hallPhotos.isEmpty) {
          _snack('Please upload at least one hall photo.'); return false;
        }
        if (_data.bankNameController.text.trim().isEmpty) {
          _snack('Please enter bank name.'); return false;
        }
        if (_data.bankAccController.text.trim().isEmpty) {
          _snack('Please enter account number.'); return false;
        }
        if (_data.ntnFile == null) {
          _snack('Please upload NTN document.'); return false;
        }
        if (_data.licenseFile == null) {
          _snack('Please upload business license.'); return false;
        }
        return true;
      default:
        return true;
  ...
  Future<void> _submitRegistration() async {
    ...
    if (uid == null) { _snack('Please log in first.'); return; }
    ...
    if (hallId == null) {
      ...
      _snack('Failed to submit. Please check your connection and try again.', isError: true);
      return;
    }
...
class _BasicDetailsStepState extends State<BasicDetailsStep> {
  Future<void> _pickCnic(bool isFront) async {
    ...
    setState(() {
      if (isFront) widget.data.cnicFront = picked;
      else         widget.data.cnicBack  = picked;
    });
    widget.onChanged();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _sectionTitle('Basic Details', 'Fill in your personal contact details.', isWide),
        SizedBox(height: isWide ? 16 : 24),
        if (isWide) ...[
          Row(children: [
            Expanded(child: _field('Full Name', d.nameController, 'Enter your Full Name', isWide: isWide)),
            const SizedBox(width: 16),
            Expanded(child: _field('Phone Number', d.phoneController, '03**-*******',
          ...
          Row(children: [
            Expanded(child: _field('Email', d.emailController, 'Enter your Email',
              type: TextInputType.emailAddress, isWide: isWide)),
            const SizedBox(width: 16),
            Expanded(child: _field('CNIC', d.cnicController, 'CNIC in format *****-*******-*',
              type: TextInputType.number, isWide: isWide)),
          ]),
        ] else ...[
          _field('Full Name', d.nameController, 'Enter your Full Name'),
          _field('Phone Number', d.phoneController, '03**-*******',
            type: TextInputType.phone,
            formatters: [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(11)],
          ),
          _field('Email', d.emailController, 'Enter your Email', type: TextInputType.emailAddress),
          _field('CNIC', d.cnicController, 'CNIC in format *****-*******-*', type: TextInputType.number),
        ],
        ...
        Row(children: [
          Expanded(child: _uploadBox('Front Side CNIC', d.cnicFront, () => _pickCnic(true),  isWide: isWide)),
          SizedBox(width: isWide ? 12 : 15),
          Expanded(child: _uploadBox('Back Side CNIC',  d.cnicBack,  () => _pickCnic(false), isWide: isWide)),
        ]),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        if (isWide) ...[
          Row(children: [
            Expanded(child: _field('Hall Name', d.hallNameController, 'Enter your Hall Name', isWide: isWide)),
            const SizedBox(width: 16),
            Expanded(child: _field('Hall Rent (Rs.)', d.rentController, "Enter Hall's Rent",
              type: TextInputType.number, isWide: isWide)),
          ]),
        ] else ...[
          _field('Hall Name', d.hallNameController, 'Enter your Hall Name'),
          _field('Hall Rent (Rs.)', d.rentController, "Enter Hall's Rent", type: TextInputType.number),
        ],
        Text('Hall Location',
          style: TextStyle(fontSize: isWide ? 12 : 13, fontWeight: FontWeight.w600)),
        SizedBox(height: isWide ? 6 : 8),
        Row(children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              width:  isWide ? 40 : 48,
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [Color(0xFFF47C20), Color(0xFFFFD166)]),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Icon(Icons.my_location, color: Colors.white, size: isWide ? 18 : 22),
            ),
          ),
        ]),
        ...
        SizedBox(height: isWide ? 12 : 16),
        _field('Hall Description', d.descController, 'Enter your Hall Description',
          maxLines: isWide ? 4 : 5, isWide: isWide),
      ],
    );
  }
}
...
class _UploadsPayoutsStepState extends State<UploadsPayoutsStep> {
  ...
  Future<void> _pickDocument(bool isNtn) async {
    try {
      ...
      final XFile xf;
      if (kIsWeb) {
        xf = XFile.fromData(pf.bytes!, name: pf.name, mimeType: _mimeFromName(pf.name));
      } else {
        xf = XFile(pf.path!);
      }
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          ...
          child: Row(children: [
            ...d.hallPhotos.asMap().entries.map((e) => Padding(
              padding: const EdgeInsets.only(right: 8),
              child: Stack(children: [
                ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Image.memory(e.value.bytes, width: thumbW, height: thumbH, fit: BoxFit.cover),
                ),
                Positioned(
                  top: 4, right: 4,
        ...
        if (isWide)
          Row(children: [
            ...
            const SizedBox(width: 16),
            Expanded(child: _field('Bank Account Number', d.bankAccController, 'Enter your Account Number',
              type: TextInputType.number, isWide: isWide)),
          ])
        else ...[
        ...
        Row(children: [
          ...
          SizedBox(width: isWide ? 12 : 15),
          Expanded(child: _docUploadBox('Business License',  d.licenseFile, () => _pickDocument(false), isWide: isWide)),
        ]),
      ],
    );
  }
...
class _MenuServicesStepState extends State<MenuServicesStep> {
  ...
  Widget _webAddButton(String label, VoidCallback onTap) => SizedBox(
    ...
    child: OutlinedButton.icon(
      ...
      label: Text(label,
        style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFFF47C20))),
      style: OutlinedButton.styleFrom(
        ...
        RichText(
          text: TextSpan(
            ...
            children: [
              ...
              TextSpan(text: '(If Any)', style: TextStyle(color: Colors.grey[400])),
            ],
          ),
        ),
        SizedBox(height: isWide ? 8 : 10),
...
class _ReviewSubmitStepState extends State<ReviewSubmitStep> {
  ...
  Widget build(BuildContext context) {
    ...
    final card1 = _reviewCard('Basic Details', 0, widget.onEditStep, isWide, [
      ...
      _reviewRow(Icons.photo,   'CNIC Photos',
        '${d.cnicFront != null ? '✓' : '✗'} Front  ${d.cnicBack != null ? '✓' : '✗'} Back', isWide),
    ]);
    final card2 = _reviewCard('Hall Details', 1, widget.onEditStep, isWide, [
      ...
      _reviewRow(Icons.location_on, 'Location',  d.locationController.text, isWide),
      _reviewRow(Icons.groups,      'Capacity',
        '${d.minController.text} – ${d.maxController.text} Guests', isWide),
      _reviewRow(Icons.payments, 'Rent', 'Rs. ${d.rentController.text}/Event', isWide),
    ]);
    final card3 = _reviewCard('Uploads & Payouts', 2, widget.onEditStep, isWide, [
      _reviewRow(Icons.photo_library,   'Hall Photos',
        '${d.hallPhotos.length} photo(s) selected', isWide),
      _reviewRow(Icons.account_balance, 'Bank',
        '${d.bankNameController.text} • ${d.bankAccController.text}', isWide),
      _reviewRow(Icons.assignment, 'Documents',
        '${d.ntnFile != null ? '✓' : '✗'} NTN  ${d.licenseFile != null ? '✓' : '✗'} License', isWide),
    ]);
    final card4 = _reviewCard('Menu & Services', 3, widget.onEditStep, isWide, [
      if (d.menuItems.isEmpty && d.serviceItems.isEmpty)
        Padding(
          ...
          child: Text('No items added.',
            style: TextStyle(color: Colors.grey[500], fontSize: isWide ? 12 : 14)),
        ),
      if (d.menuItems.isNotEmpty) ...[
        ...
        ...d.menuItems.map((i) =>
          _itemRow(i['name'] ?? '', 'Rs. ${i['price']}/${i['priceUnit']}', isWide)),
      ...
      if (d.serviceItems.isNotEmpty) ...[
        Text('Services',
          style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey,
            fontSize: isWide ? 11 : 13)),
        SizedBox(height: isWide ? 4 : 6),
        ...d.serviceItems.map((s) => _itemRow(s['name'] ?? '', 'Rs. ${s['price']}', isWide)),
      ],
    ]);
      ...
      children: [
        ...
        if (isWide)
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(child: Column(children: [card1, const SizedBox(height: 12), card3])),
              const SizedBox(width: 12),
              Expanded(child: Column(children: [card2, const SizedBox(height: 12), card4])),
            ],
          )
        else ...[
          card1, const SizedBox(height: 16),
          card2, const SizedBox(height: 16),
        ...
        GestureDetector(
          ...
          child: Row(children: [
            AnimatedContainer(
              ...
              child: _isConfirmed
                  ? Icon(Icons.check, size: isWide ? 13 : 16, color: Colors.white)
                  : null,
            ),
            const SizedBox(width: 12),
            Expanded(
        ...
        SizedBox(
          ...
          height: isWide ? 42 : 52,
          child: ElevatedButton(
            onPressed: (_isConfirmed && !widget.isSubmitting) ? widget.onSubmit : null,
            style: ElevatedButton.styleFrom(
              backgroundColor: const Color(0xFFF47C20),
  ...
  Widget _itemRow(String name, String price, bool isWide) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        Text(price,
          style: TextStyle(fontWeight: FontWeight.bold,
            color: const Color(0xFFF47C20), fontSize: isWide ? 12 : 14)),
      ],
    ),
  );
}
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('✓ PDF uploaded',
                          style: TextStyle(
                            fontSize: 9, color: Color(0xFFF47C20), fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                )
...
Widget _reviewRow(IconData icon, String label, String value, bool isWide) => Padding(
