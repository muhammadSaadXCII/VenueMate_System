...
class _ManageUserDetailsScreenState extends State<ManageUserDetailsScreen> {
  ...
  // Stream the user so isDisabled changes reflect immediately
  ...
  Stream<UserModel?> _streamUser(String uid) {
  ...
  // ── Fetch booking stats for this user ─────────────────────────────────────
  ...
  Future<void> _toggleStatus() async {
        ...
        content: Text(
          willDisable
              ? 'This user will be blocked from logging in '
                    'and will not be able to use VenueMate.'
    ...
    String? error;
    if (willDisable) {
      error = await UserService.disableUser(user.uid);
    } else {
      error = await UserService.enableUser(user.uid);
    }
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<UserModel?>(
      ...
      builder: (context, snap) {
        if (snap.data != null) _user = snap.data;
        final user = _user ?? widget.user;
