...
class NotificationService {
  ...
  static final _notificationsRef = FirebaseFirestore.instance.collection(
    'notifications',
  );
  ...
  /// Uses set+merge so it works even if fcmToken field never existed before.
  ...
  // ════════════════════════════════════════════════════════════════════════════
  //  3b. INTERNAL: NOTIFY ALL SYSTEM ADMINS
  //  Queries Firestore for every user with role == 'system_admin' and sends
  //  a push + in-app notification to each one.
  // ════════════════════════════════════════════════════════════════════════════

  static Future<bool> _notifyAllSystemAdmins({
    required String title,
    required String body,
    required NotificationType type,
    String referenceId = '',
    ...
    try {
      ...
      // Send to every system admin in parallel
      final results = await Future.wait(
        snap.docs.map(
          (doc) => _sendNotification(
            recipientUid: doc.id,
            title: title,
            body: body,
            type: type,
      ...
      return results.any((r) => r); // true if at least one succeeded
    } catch (e) {
      print('❌ FCM _notifyAllSystemAdmins error: $e');
      return false;
    }
  }
  ...
  /// Notify customer that their event has been marked as completed by hall admin.
  static Future<bool> sendBookingCompleted({
    required String customerUid,
    required String bookingId,
    ...
    body:
        'Your event "$eventName" at $hallName has been marked as completed. We hope it was a great experience!',
    type: NotificationType.bookingConfirmed,
    referenceId: bookingId,
    referenceType: 'booking',
  );
  ...
  /// Notify hall admin that the customer accepted their refund receipt.
  static Future<bool> sendRefundAccepted({
    required String hallAdminUid,
    required String bookingId,
    required String customerName,
    required String amount,
    ...
    body: '$customerName has confirmed receipt of the Rs. $amount refund.',
    type: NotificationType.refundProcessed,
    referenceId: bookingId,
    referenceType: 'payment',
  );

  /// Notify hall admin that the customer rejected their refund receipt.
  static Future<bool> sendRefundRejected({
    required String hallAdminUid,
    required String bookingId,
    required String customerName,
    ...
    recipientUid: hallAdminUid,
    title: 'Refund Receipt Rejected',
    body: '$customerName rejected the refund receipt. Reason: $reason',
    type: NotificationType.paymentRejected,
    referenceId: bookingId,
    referenceType: 'payment',
  );
  ...
  /// Notify ALL system admins that a new hall registration was submitted.
  static Future<bool> sendRegistrationSubmitted({
    required String systemAdminUid, // kept for API compatibility, ignored
    required String hallId,
    required String hallName,
    required String ownerName,
  }) async {
    return _notifyAllSystemAdmins(
  ...
  static Future<bool> sendHallApproved({
    ...
    title: 'Hall Approved!',
    body: 'Congratulations! "$hallName" is now live on VenueMate.',
  ...
  static Future<bool> sendFeedbackReceived({
    required String hallAdminUid,
    required String hallId,
    required String hallName,
    required String customerName,
    ...
    body: '$customerName rated $hallName $rating/5 stars.',
  ...
  /// Notify ALL system admins that a user filed a complaint.
